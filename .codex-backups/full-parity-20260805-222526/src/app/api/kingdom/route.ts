...
export async function GET(request: NextRequest) {
  try {
    ...
    const [kingdom, members, races] = await Promise.all([
    ...
    return NextResponse.json({ inKingdom: true, kingdom, members: memberList, myVote, diplomacy }, { status: 200 });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, kingdomId, targetPID, relationId } = await request.json();
