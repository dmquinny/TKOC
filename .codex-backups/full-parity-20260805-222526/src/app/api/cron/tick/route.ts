  ...
  try {
    ...
    for (const effect of activeLegacySpells) {
      ...
      if (effect.type === 'DoomSpell') {
        ...
        const soldiers = target.military.find(unit => unit.type.category === 'soldiers');
        if (soldiers) {
          // The modern schema has no legacy dead-unit ledger. Reconstituting a
          // percentage of the current host is the closest durable equivalent.
          const raised = Math.floor(soldiers.num * (.025 + Math.random() * .175) * strength);
          if (raised) await tx.militaryUnit.update({ where: { id: soldiers.id }, data: { num: { increment: raised } } });
        }
      }
    }
    ...
    for (const p of provinces) {
      ...
      const housing = Math.floor((acres * ACRE_CAPACITY
        + p.buildings.reduce((sum, building) => {
          const name = canonicalBuildingName(building.type.className);
          const capacity = name === 'Home'
            ? HOME_CAPACITY
            : name === 'Dock'
              ? 25
              : name === 'Wall'
                ? 10
                : 20;
          return sum + building.num * (capacity - ACRE_CAPACITY);
        }, 0)
      ) * raceMods.peasantHousing * (1 + sciPct(p, 'housing') / 100));
      ...
      const scienceLevels = p.science.reduce((sum, sc) => sum + sc.level, 0);
      const militaryValue = p.military.reduce((sum, u) => sum + u.num * (u.type.attack + u.type.defense), 0);
      const newNetworth = Math.floor(
        (p.acres || 0) * 10 + newPeasants * 0.5 + totalBuildings * 25 + militaryValue * 2 + scienceLevels * 50 + (newGold + newMetal + newFood) / 1000
      );
