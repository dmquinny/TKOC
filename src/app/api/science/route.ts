...
import {
  ...
  legacyRaceScienceCategoryAllowed,
  scienceKnowledge,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const visibleRules = ORIGINAL_SCIENCES.filter(rule => (
      ...
      && legacyRaceScienceCategoryAllowed(race?.name, rule.category)
    ));
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await withSerializableTransaction(async tx => {
      ...
      if (!legacyRaceScienceCategoryAllowed(race?.name, type.category)) {
        throw new Error('That knowledge is not available to your race');
      }
      ...
      const dockMultiplier = Math.max(
        0.0001,
        1 + buildingEffect(docks, province.acres ?? 0, -1, 20),
      );
