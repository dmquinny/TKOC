...
export async function GET(request: NextRequest) {
  try {
    ...
    const [me, wizardUnit, trainingOrders, maintained] = await Promise.all([
    ...
    const [race, targets] = await Promise.all([
      me.raceId ? prisma.race.findUnique({ where: { id: me.raceId } }) : null,
      listTargets(prisma, me, 'magic'),
    ]);
    ...
    const availableWizards = Math.max(
      ...
      (wizardUnit?.num ?? 0)
        ...
        - me.outgoingAttacks.flatMap(attack => attack.armies)
          .filter(army => army.mID === wizardUnit?.mID)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await withSerializableTransaction(async tx => {
      ...
      const housingScience = me.science.reduce(
        (sum, entry) => sum + (entry.type.effect === 'housing' ? entry.level * entry.type.bonusPerLevel : 0),
        0,
      );
      const housing = legacyPeasantHousing(
        me.acres ?? 0,
        me.buildings,
        legacyRaceModifiers(myRace?.name).peasantHousing,
        housingScience,
      );
      if (legacyActionBlockedByOverpopulation(
        me.peasants ?? 0,
        me.military.reduce((sum, unit) => sum + Math.max(0, unit.num), 0),
        housing,
      )) {
        throw new Error('Your province is too overpopulated to cast spells');
      }
      ...
      const awayWizards = me.outgoingAttacks.flatMap(attack => attack.armies)
        .filter(army => army.mID === wizardUnit?.mID)
        .reduce((sum, army) => sum + army.num, 0);
      const targetAwayWizards = target.outgoingAttacks.flatMap(attack => attack.armies)
        .filter(army => army.mID === targetWizardUnit?.mID)
        .reduce((sum, army) => sum + army.num, 0);
      const readyWizards = Math.max(0, (wizardUnit?.num ?? 0) - (training?._sum.num ?? 0) - awayWizards - (maintained._sum.wizards ?? 0));
      const targetWizards = Math.max(0, (targetWizardUnit?.num ?? 0) - (targetTraining?._sum.num ?? 0) - targetAwayWizards);
      if (readyWizards < channel) throw new Error('Not enough Wizards');

      const wizardUseScience = me.science.reduce(
        (sum, entry) => sum + (entry.type.effect === 'wizardUse' ? entry.level * entry.type.bonusPerLevel : 0),
        0,
      );
      const raceRules = legacyRaceModifiers(myRace?.name);
      const wizardUse = legacyEffectMultiplier(wizardUseScience) * raceRules.wizardUse;
      const targetAcres = Math.max(1, target.acres ?? 0);
      const neededWizards = Math.max(1, Math.ceil(def.wizards * targetAcres * wizardUse));
      const manaCost = raceRules.manaCost;
      const manaUsed = Math.round(Math.max(5, def.mana * manaCost, def.mana * manaCost * targetAcres * def.wizards * wizardUse / channel));
      const hasTrade = me.science.some(entry => entry.type.className === 'TradeScience' && entry.level > 0);
      const resourceCost = {
        gold: Math.ceil(def.gold * targetAcres * (hasTrade ? .5 : 1)),
        metal: Math.ceil(def.metal * targetAcres * (hasTrade ? .75 : 1)),
        food: Math.ceil(def.food * targetAcres),
        peasants: Math.ceil(def.peasants * targetAcres),
      };
      ...
      const towerCount = (buildings: Array<{ num: number; type: { className: string | null } }>) => buildings.reduce(
        (sum, entry) => sum + (canonicalBuildingName(entry.type.className) === 'Wizard Tower' ? entry.num : 0),
        0,
      );
      const towerChancePercent = buildingEffect(towerCount(me.buildings), me.acres ?? 0, 1) * 100;
      const castingPower = legacyEffectMultiplier((myRace?.magicBonus ?? 100) - 100)
        * legacyEffectMultiplier(towerChancePercent)
        * legacyEffectMultiplier(scienceMagic)
        * legacyEffectMultiplier(advisorMagic);
      const targetTowerPercent = buildingEffect(towerCount(target.buildings), target.acres ?? 0, 1) * 100;
      const protection = def.target === 'hostile'
        ? legacyEffectMultiplier((legacyRaceModifiers(targetRace?.name).magicProtection - 1) * 100)
          * legacyEffectMultiplier(targetTowerPercent)
          * legacyEffectMultiplier(activeSpellPercent(target.effects, 'magicProtection'))
          * legacyEffectMultiplier(advisorProtection)
        : 1;
      const randomMana = Math.round(randomInt(1, 100) * protection / castingPower);
      const randomDuration = Math.round(randomInt(2, 72) * castingPower / protection);
      const ownWpaRoll = def.target === 'hostile'
        ? randomInt(1, Math.max(1, Math.round(readyWizards / Math.max(1, me.acres ?? 0) * 1000 * castingPower * 3)))
        : 0;
      const targetWpaRoll = def.target === 'hostile'
        ? randomInt(1, Math.max(1, Math.round(targetWizards / targetAcres * 1000 * protection)))
        : 0;
      ...
      if (!success) {
        ...
        if (wizardUnit && randomInt(1, 3) === 1) {
          ...
          lost = Math.min(lost, wizardUnit.num);
          await tx.militaryUnit.update({ where: { id: wizardUnit.id }, data: { num: { decrement: lost } } });
          await recordDeadMilitary(tx, pID, wizardUnit.mID, lost);
        }
        const difficulty = Math.round(def.wizards * 3);
        await tx.province.update({
          where: { id: pID },
          data: { magicRep: Math.max(0, (me.magicRep ?? 0) - difficulty) },
        });
      ...
      if (def.mode === 'timed') {
        ...
        for (const unit of target.military) {
          ...
          if (loss) {
            ...
            await recordDeadMilitary(tx, targetID, unit.mID, loss);
          }
          troopsLost += loss;
        }
        await tx.province.update({
        ...
        if (apocalypse && wizardUnit) {
          ...
          if (backfire) {
            await tx.militaryUnit.update({ where: { id: wizardUnit.id }, data: { num: { decrement: backfire } } });
            await recordDeadMilitary(tx, pID, wizardUnit.mID, backfire);
          }
        }
      ...
      await tx.province.update({
        where: { id: pID },
        data: { magicRep: { increment: Math.round(def.wizards * 3) } },
      });
    ...
    const expected = /Target|spell|race|science|magic|Wizards|mana|resources|kingdom|Protection|duration|overpopulated/i.test(message);
