...
export async function GET(request: NextRequest) {
  try {
    ...
    const targets = await listTargets(prisma, me, 'thievery');
  ...
  try {
    ...
    const result = await withSerializableTransaction(async tx => {
      ...
      const housing = legacyPeasantHousing(
        me.acres ?? 0,
        me.buildings,
        legacyRaceModifiers(race?.name).peasantHousing,
        housingScience,
      );
      const totalMilitary = me.military.reduce((sum, unit) => sum + Math.max(0, unit.num), 0);
      if (legacyActionBlockedByOverpopulation(me.peasants ?? 0, totalMilitary, housing)) {
        throw new Error('Your province is too overpopulated to conduct thievery operations');
      }
      ...
      if (!success) {
        ...
        await recordDeadMilitary(tx, pID, thiefUnit.mID, lost);
        // Legacy GetIdentification identifies the attacker on two outcomes out
        // of three; only a roll of 2 conceals the province.
        const identified = randomInt(1, 3) !== 2;
      ...
      if (operation.className === 'SpyOnProvince') {
        const scienceTypeCount = await tx.scienceType.count({
          where: { className: { not: 'EndGameScience' } },
        });
        Object.assign(revealed, {
          ...
          knowledge: `${Math.floor((target.science.filter(entry => entry.level > 0).length / Math.max(1, scienceTypeCount)) * 100)}%`,
          gold: target.gold,
          food: target.food,
          metal: target.metal,
        ...
        if (randomInt(1, 5) === 1) {
          ...
          if (thievesLost) {
            await tx.militaryUnit.update({ where: { id: thiefUnit.id }, data: { num: { decrement: thievesLost } } });
            await recordDeadMilitary(tx, pID, thiefUnit.mID, thievesLost);
          }
        }
        ...
        const news = target.kiID
          ? await tx.kingdomNews.findMany({
              where: { kiID: target.kiID },
        ...
        const provinces = target.kiID
          ...
          : [];
        revealed.kingdom = { name: kingdom?.name ?? 'Unassigned', provinces, news };
      ...
      const reputation = legacyThieveryReputation(operation.className);
      if (reputation > 0) {
        await tx.province.update({ where: { id: pID }, data: { reputation: { increment: reputation } } });
        await tx.province.update({
          where: { id: targetID },
          data: { reputation: Math.max(0, (target.reputation ?? 0) - reputation) },
        });
      }
      if (thievesLost > 0) {
    ...
    const expected = /Target|Protection|influence|science|Thieves|province|operation|overpopulated/i.test(message);
