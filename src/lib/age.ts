...
import { legacyMagicRank, legacyThieveryRank } from '@/lib/legacy-networth';
...
export async function endAgeInTransaction(tx: Prisma.TransactionClient, currentAge: number) {
  ...
  const [kingdoms, users] = await Promise.all([
    tx.kingdom.findMany({ select: { id: true, name: true } }),
    tx.user.findMany({ where: { pID: { gt: 0 } }, select: { id: true, pID: true, username: true } }),
  ]);
  const kName = new Map(kingdoms.map(k => [k.id, k.name]));
  const userByProvince = new Map(users.map(user => [user.pID, user]));
  if (top.length) {
    await tx.ageResult.createMany({
      data: top.map((p, i) => ({
        ...
        rank: i + 1,
        pID: p.id,
        userID: userByProvince.get(p.id)?.id ?? 0,
        userName: userByProvince.get(p.id)?.username ?? null,
        kiID: p.kiID,
        provinceName: p.provinceName ?? 'Unknown',
        rulerName: p.rulerName ?? 'Unknown',
        networth: p.networth ?? 0,
        acres: p.acres ?? 0,
        thieveryRank: legacyThieveryRank(p.reputation),
        thieveryPoints: p.reputation,
        magicRank: legacyMagicRank(p.magicRep),
        magicPoints: p.magicRep,
        militaryPoints: p.militaryRep,
  ...
  await tx.battleReport.deleteMany({});
  ...
  await tx.kingdomNews.deleteMany({});
