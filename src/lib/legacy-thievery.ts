...
import { legacyEffectMultiplier } from './legacy-effects';
...
const LEGACY_THIEVERY_REPUTATION: Record<string, number> = {
  SpyOnProvince: 1,
  SpyOnSciences: 1,
  RobSupplies: 2,
  SpyOnMilitary: 1,
  ...
  Infiltrate: 3,
  SpyOnKingdom: 1,
  AssasinateCouncil: 10,
  SpyOnBuildings: 0,
  Screen: 3,
  Investigate: 0,
  AssasinateMilitary: 5,
  SabotageArmy: 1,
  Riots: 1,
};

export function legacyThieveryReputation(className: string): number {
  return LEGACY_THIEVERY_REPUTATION[className] ?? 1;
}
